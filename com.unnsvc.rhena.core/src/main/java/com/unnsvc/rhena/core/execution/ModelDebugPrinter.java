...
public class ModelDebugPrinter {
	...
	public void debugPrint(IRhenaModule model, EExecutionType type) throws RhenaException {
		...
		for (ILifecycleDeclaration lifecycle : graphResolver.getLifecycles()) {
			...
			context.materialiseModel(lifecycle.getContext().getModuleEdge().getTarget())
			...
			lifecycle.getProcessors().forEach(processor -> {
				try {
					context.materialiseModel(processor.getModuleEdge().getTarget()).visit(new LoggingVisitor(processor.getModuleEdge().getExecutionType(), context, 0, "processor"));
				} catch (RhenaException e) {

					e.printStackTrace();
				}
			});
			context.materialiseModel(lifecycle.getGenerator().getModuleEdge().getTarget())
