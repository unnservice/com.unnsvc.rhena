...
public class DefaultProcessor implements IProcessor {
	...
	private void compile(IRhenaModule module, IResource resource, ExecutionType type) throws RhenaException {
		...
		for (IRhenaEdge edge : module.getDependencyEdges()) {
			...
			if (edge.getExecutionType().equals(type)) {
				deps.addAll(module.visit(new RhenaDependencyCollectionVisitor(context, ExecutionType.FRAMEWORK, TraverseType.SCOPE)).getDependenciesURL());
			}
		}
